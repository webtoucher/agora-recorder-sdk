...
int napi_get_value_string_utf8_(const Local<Value> &str, char *buffer, uint32_t len)
{
    ...
    if (!buffer)
    {
        return str.As<String>()->Utf8Length(Isolate::GetCurrent());
    }
    else
    {
        int copied = str.As<String>()->WriteUtf8(Isolate::GetCurrent(), buffer, len - 1, nullptr, String::REPLACE_INVALID_UTF8 | String::NO_NULL_TERMINATION);
        buffer[copied] = '\0';
        return copied;
    }
}
...
napi_status napi_get_value_uint32_(const Local<Value> &value, uint32_t &result)
{
    ...
    result = value->Uint32Value(Isolate::GetCurrent()->GetCurrentContext()).FromJust();
    return napi_ok;
}
...
napi_status napi_get_value_bool_(const Local<Value> &value, bool &result)
{
    if (!value->IsBoolean())
        return napi_invalid_arg;
    result = value->BooleanValue(Isolate::GetCurrent());
    return napi_ok;
}
...
napi_status napi_get_value_int32_(const Local<Value> &value, int32_t &result)
{
    if (!value->IsInt32())
        return napi_invalid_arg;
    result = value->Int32Value(Isolate::GetCurrent()->GetCurrentContext()).FromJust();
    return napi_ok;
}
...
napi_status napi_get_value_double_(const Local<Value> &value, double &result)
{
    ...
    result = value->NumberValue(Isolate::GetCurrent()->GetCurrentContext()).FromJust();
    return napi_ok;
}
...
{
    return String::NewFromUtf8(isolate, value ? value : "").FromMaybe(Local<Value>());
}
