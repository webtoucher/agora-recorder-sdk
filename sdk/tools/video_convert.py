...
class AudioClip:
    ...
    def print_ffmpeg(self, output_file, offset_time):
        if self.num >= 1:
            str = "ffmpeg -hide_banner " + self.print_filename(offset_time)
            ...
            str = "ffmpeg -hide_banner -i \'./%s\' -c:a copy \'./%s\'" % (self.filename[0], output_file)
        else:
            str = ""
...
class VideoClip:
    ...
    def print_filter(self, offset_time):
        ...
        for i in range(self.num):
            ...
            if need_repeat_flag:
                eof_action = "pass"
                if self.repeat[i] == 1 and len(self.start_time) >= 2:
    ...
    def print_ffmpeg(self, output_file, offset_time):
        if self.audio_file == "":
            str = "ffmpeg -hide_banner -f lavfi -i anullsrc "
        else:
            str = "ffmpeg -hide_banner -i \'./%s\' " % self.audio_file
        ...
        str = str + " %s -c:v libx264 -r %d -preset veryfast -shortest -to %f -movflags faststart -y \'./%s\'" % (map_option, dest_fps, self.max_length() - offset_time, output_file)
        str = str + " 2>&1 | tee -a convert.log"
        print "=================================video ffmpeg ========================"
        print str
        return str
    ...
    def print_ffmpeg_with_resolution(self, output_file, offset_time, target_width, target_height):
        if self.audio_file == "":
            str = "ffmpeg -hide_banner -f lavfi -i anullsrc=channel_layout=mono:sample_rate=48000 "
        else:
            str = "ffmpeg -hide_banner -i \'./%s\' " % self.audio_file
        ...
        str = str + " -movflags faststart -y \'./%s\'" % output_file
        str = str + " 2>&1 | tee -a convert.log"
        print "=================================video ffmpeg ========================"
        print str
        return str
